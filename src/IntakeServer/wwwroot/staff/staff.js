...
(function () {
  ...
  function adminAccess(content) {
    api('GET', '/api/staff/access').then(function (a) {
      ...
      if (a.localOnly) content.appendChild(h('div', { class: 'alert warn' }, h('div', null, h('strong', { text: 'Máy chủ đang chỉ phục vụ trên chính máy này (localhost).' }),
    ...
    api('GET', '/api/staff/users').then(function (d) {
      ...
      (d.items || []).forEach(function (u) {
        body.appendChild(h('tr', null, h('td', { class: 'mono', text: u.username }), h('td', { text: u.displayName }),
          h('td', { text: u.role === 'admin' ? 'Quản trị' : (u.role === 'doctor' ? 'Bác sĩ' : 'Điều dưỡng') }),
  ...
  function userForm(content, existing) {
    ...
    var role = h('select', { id: 'uf-r' }, h('option', { value: 'nurse', text: 'Điều dưỡng' }), h('option', { value: 'doctor', text: 'Bác sĩ' }), h('option', { value: 'admin', text: 'Quản trị' }));
